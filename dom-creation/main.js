...
function renderPokemon(pokemon) {
  ...
  $columnContainer.className = 'column-third';
  $pokemonCard.className = 'pokemon-card';
  $pokemonImage.setAttribute('src', pokemon.imageUrl);
  $cardTextContainer.className = 'pokemon-card-text';
  $pokemonName.textContent = pokemon.name;
  $pokemonNumber.textContent = pokemon.number;
  $pokemonDescription.textContent = pokemon.description;

  $cardTextContainer.append($pokemonName, $pokemonNumber, $pokemonDescription);
  $pokemonCard.append($pokemonImage, $cardTextContainer);
  $columnContainer.appendChild($pokemonCard);
  return $columnContainer;
}

var $rowContainer = document.querySelector('.row');
for (var i = 0; i < pokedex.length; i++) {
  $rowContainer.appendChild(renderPokemon(pokedex[i]));
}
